...
SPARK_Y       = BARS_Y + BAR_ROWS * (BAR_H + GAP)     # = 12 + 3*12 = 48
SPARK_H       = H - SPARK_Y - 1                       # = 15
...
def render_frame(m: dict, cpu_history: deque) -> Image.Image:
    ...
    # Time on left, date on right
    draw.text((2, 1),  time_str, font=font, fill=(255, 220, 80))   # warm yellow
    rw = int(text_w(draw, date_str, font))
    draw.text((W - rw - 2, 1), date_str, font=font, fill=LABEL_C)
    # Divider below time bar
    ...
    for i, (label, pct, right) in enumerate(rows):
        draw_bar_row(draw, font, BARS_Y + i * (BAR_H + GAP), label, pct, right)
